...
if [ -f "$WIKI_PATH/index.md" ]; then
  grep -oh '\[\[[^]]*\]\]' "$WIKI_PATH"/*.md 2>/dev/null | sort -u | sed 's/\[\[//;s/\]\]//' | while read -r page; do
...
for f in "$WIKI_PATH"/*.md; do
  ...
  refs=$(grep -rl "\[\[$basename_f\]\]" "$WIKI_PATH"/*.md 2>/dev/null | grep -v "$(basename "$f")" | wc -l | tr -d ' ')
  if [ "$refs" -eq 0 ]; then
...
unique_refs=$(grep -oh '\[\[[^]]*\]\]' "$WIKI_PATH"/*.md 2>/dev/null | sort -u | wc -l | tr -d ' ')
...
where_count=$(grep -rl "关键文件\|Key Files\|Source Files" "$WIKI_PATH"/*.md 2>/dev/null | wc -l | tr -d ' ')
why_count=$(grep -rl "架构决策\|Design Decision\|Why" "$WIKI_PATH"/*.md 2>/dev/null | wc -l | tr -d ' ')
whatnot_count=$(grep -rl "反模式\|Anti-pattern\|What Not" "$WIKI_PATH"/*.md 2>/dev/null | wc -l | tr -d ' ')
